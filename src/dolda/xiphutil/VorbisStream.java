...
public class VorbisStream {
	 ...
	 * key-value pairs decoded as normal <code>java.lang.String</code>s. The keys
	 * are guaranteed to be lower case and interned, and the values are exactly as
	 * provided from the Vorbis comment. Please see the
	 ...
	 *
	 * @exception java.io.IOException if the <code>PacketStream</code> itself
	 * throws an <code>IOException</code>.
	 * @exception FormatException if a format error is found in the input.
	 */
	 ...
	 *
	 * @exception java.io.IOException if the <code>InputStream</code> itself
	 * throws an <code>IOException</code>.
	 * @exception FormatException if a format error is found in the input.
	 */
	 ...
	 * of pairwise equal length, and to contain at least one sample. Therefore, if
	 * <code>buf</code> is a returned value from this function, the expression
	 * <code>buf[0].length</code> is guaranteed to work for fetching the number of
	 * samples from this cycle.
	 *
	 * @return The decoded data, or <code>null</code> when the stream ends.
	 *
	 * @exception java.io.IOException if the backing input stream itself throws an
	 ...
	 *
	 * @return A <code>java.io.InputStream</code> object from which the PCM stream
	 * can be read.
	 */
	 ...
	 * testing the functionality of this <code>VorbisStream</code> implementation
	 * or individual Ogg/Vorbis files. It takes one command-line argument to be
	 * interpreted as the path to an Ogg/Vorbis file to be decoded into PCM data.
	 * The PCM data is output to standard output.
	 *
	 * You can use the <code>sox</code> program to play Ogg/Vorbis files with this
	 * class as well:
	 * <pre>
